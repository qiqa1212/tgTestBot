...
def new_list(file):
   file = str(file)
   file = list(file)
   return chek_chek(file, 'New list:')
...
def find_index(my_list):
   ...
   for i in range(len(my_list)):
      if my_list[i] == '{' or my_list[i] == '}':
         my_ind.append(i)
   return chek_chek(my_ind, 'Index:')

def chek_chek(var, text):
   if len(var)> 0:
      print(text, "OK")
      return var
   else:
...
def takeInfo(list, ind):
   for i in range(len(ind)):
      print(list[ind[i]], end='')
   #return rec_find(list, ind)
   
   
steps = []
def rec_find(list, ind):
   group_list = [ind[0],ind[-1]]
   for i in range(len(ind)):
      pass
   print(group_list)
...
def find(file):
   list_str = new_list(file)
   indexes = find_index(list_str)
   print(indexes)
   info =  takeInfo(list_str, indexes)
